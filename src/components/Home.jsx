...
const Home = () => {
  ...
  const nossasLinhasPath = (process.env.PUBLIC_URL || "") + "/assets/img/nossas-linhas/full/";
  const CATEGORIAS = [
    { nome: "Linha Premium",       rota: "/products/linha-premium",       imagem: "premium.webp"             },
    { nome: "Luminárias e Postes", rota: "/products/luminarias-e-postes", imagem: "luminarias-e-postes.webp" },
    { nome: "Linha Construção",    rota: "/products/construcao",          imagem: "construcao.webp"          },
    /*{ nome: "Linha Piscina",    rota: "/products/piscina",              imagem: "piscina.webp"          },*/
  ];
  ...
  <div className="home-main-container" style={{ backgroundColor: '#fff', width: '100%', overflowX: 'hidden' }}>
    {/* 1. SLIDER PRINCIPAL */}
    <section className="home-slider" style={{height: '55vh', minHeight: '380px', marginTop: '70px', marginBottom: '0'}}>
      <Swiper
    ...
    <section className="linhas-section">
      ...
      <div className="categorias-flex-container">
        {CATEGORIAS.map(cat => (
          <Link key={cat.rota} to={cat.rota} className="categoria-card">
            <div className="categoria-bg" style={{ backgroundImage: `url("${nossasLinhasPath}${cat.imagem}")` }}>
      ...
      <div style={{ textAlign: 'center', marginTop: '15px' }}>
        <Link to="/products" className="btn-visualizar-todos">Conheça Nossos Produtos</Link>
      </div>
    </section>
    {/* 4. SOBRE A FÁBRICA */}
    <section style={{
